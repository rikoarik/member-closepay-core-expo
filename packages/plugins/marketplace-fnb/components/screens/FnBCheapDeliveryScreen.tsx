/**
 * FnBCheapDeliveryScreen – Daftar merchant dengan ongkir murah / gratis.
 */
...
/** Urutkan: Gratis ongkir dulu, lalu Diskon ongkir, lalu lainnya */
function cheapDeliverySort(a: DiscoveryMerchant, b: DiscoveryMerchant): number {
  const fa = a.deliveryFee.toLowerCase();
  const fb = b.deliveryFee.toLowerCase();
  const freeA = fa.includes("gratis") ? 0 : 1;
  const freeB = fb.includes("gratis") ? 0 : 1;
  if (freeA !== freeB) return freeA - freeB;
  const discA = fa.includes("diskon") ? 0 : 1;
  const discB = fb.includes("diskon") ? 0 : 1;
  return discA - discB;
}

export const FnBCheapDeliveryScreen: React.FC = () => {
  ...
  const merchants = useMemo(
    () => [...FNBDISCOVERY_MERCHANTS].sort(cheapDeliverySort),
    []
  );
      ...
      >
        ...
        <View style={styles.merchantBody}>
          ...
          <View style={styles.ratingRow}>
            ...
            <Text
              style={[styles.merchantMeta, { color: colors.primary }]}
              numberOfLines={1}
            >
              {item.deliveryFee}
            </Text>
...
export default FnBCheapDeliveryScreen;
